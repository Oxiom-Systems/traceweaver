...
def commit_index_tree(repo: str, message: str) -> str:
    """Refuse canonical commit/ref mutation from the packaged CE work engine."""
    del repo, message
    raise Operational(
        "REFUSED",
        "TraceWeaver package forbids unit workspace commit or ref advancement; "
        "return verified uncommitted changes to tw-commit-push-pr",
    )
...
def cmd_checkpoint_plan(args) -> tuple[str, dict]:
    with locked_manifest(args.run_id, write=True) as doc:
        ...
        event(doc, "plan-checkpoint-held-uncommitted", detail={
            "head": info["head"],
            "path": plan_rel,
            "digest": doc["plan"]["digest"],
        })
        return "HELD_UNCOMMITTED", {
            "checkpoint": None,
            "head": info["head"],
            "path": plan_rel,
            "digest": doc["plan"]["digest"],
