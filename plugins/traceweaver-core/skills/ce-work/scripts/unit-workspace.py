...
def main(argv: list[str]) -> int:
    ...
    if args.command in HELD_NEW_WORK_COMMANDS:
        print("REFUSED")
        sys.stderr.write(
            "unit-workspace: TraceWeaver alpha holds external unit-workspace "
            "execution; use native inline/subagent work and the controlled "
            "publication wrapper\n"
        )
        return 1
    try:
