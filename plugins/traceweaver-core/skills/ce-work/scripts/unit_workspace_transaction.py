...
def cmd_integrate(args) -> tuple[str, dict]:
    raise Operational(
        "REFUSED",
        "TraceWeaver alpha holds external unit integration; use native inline/subagent work",
    )


def _held_cmd_integrate(args) -> tuple[str, dict]:
    ...
    try:
        ...
        # TraceWeaver package overlay: keep the verified canonical diff but
        # return it unstaged and uncommitted to the controlled publication
        # wrapper. The upstream CE controller normally creates a canonical
        # commit here; that would bypass TraceWeaver's exact-tree capsule gate.
        git(repo, "reset", "--mixed", before["head"])
        changed_paths = sorted(status_paths(repo))
        cmd_integration_release(_args(run_id=args.run_id, unit_id=args.unit_id, lock_token=token))
        token = None
        return "UNIT_VERIFIED_UNCOMMITTED", {
            "unit_id": args.unit_id,
            "canonical_commit": None,
            "verification_digest": evidence,
            "verification_log_retained": True,
            "cleaned_paths": cleaned_paths,
