...
def cmd_authorize_dispatch(args) -> tuple[str, dict]:
    raise Operational(
        "REFUSED",
        "TraceWeaver alpha holds external unit dispatch; use native inline/subagent work",
    )


def _held_cmd_authorize_dispatch(args) -> tuple[str, dict]:
...
def terminalize(run_id: str, unit_id: str) -> dict:
    raise Operational(
        "REFUSED",
        "TraceWeaver alpha holds external unit terminalization and synthetic transport refs",
    )


def _held_terminalize(run_id: str, unit_id: str) -> dict:
