...
Cross-model/external unit-workspace execution is held and machine-refused in
the TraceWeaver alpha package. Select native inline/subagent execution instead.
The imported routing and controller material below is retained only as CE
3.20.0 provenance and must not be followed by the packaged skill until a later
reviewed adapter replaces its commit/ref-based completion contract.
