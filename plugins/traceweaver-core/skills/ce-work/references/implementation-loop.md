...
The TraceWeaver alpha package holds and machine-refuses the imported
cross-model/unit-workspace path. Any cross-model, wave, canonical-commit,
resume, or fallback text below is inactive provenance; run this loop only with
the native inline/subagent implementation engine.
