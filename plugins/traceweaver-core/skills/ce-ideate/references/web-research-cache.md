...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `<temporary CE ideation scratch root>/<run-id>`, resolved once in SKILL.md Phase 1.
...
```bash
SCRATCH_ROOT="<temporary CE ideation scratch root>"
