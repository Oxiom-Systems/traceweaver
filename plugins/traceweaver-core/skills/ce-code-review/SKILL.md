...
- **Never edit files or externalize work.** Do not write `<temporary CE code-review artifact root>/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
- **Write a run artifact** under `<temporary CE code-review artifact root>/<run-id>/` summarizing findings, applied fixes, and advisory outputs. Include the artifact path in the structured output.
...
Artifact: <temporary CE code-review artifact root>/<run-id>/
...
**Detail enrichment (headless only):** The headless envelope includes `Why:`, `Evidence:`, and `Suggested fix:` lines. After merge (Stage 5), read the per-agent artifact files from `<temporary CE code-review artifact root>/{run_id}/` for only the findings that survived dedup and confidence gating.
...
- In interactive, autofix, and headless modes, write a per-run artifact under `<temporary CE code-review artifact root>/<run-id>/` containing:
