...
## TraceWeaver Package Hold

Cross-model code egress is held and disabled in the TraceWeaver package. Do
not run the bundled worker, even when `CROSS_MODEL_PEERS` or other CE variables
are configured. Use the in-process adversarial reviewer. The remaining text is
upstream reference material only and does not authorize egress.
...
In the TraceWeaver package, `CROSS_MODEL_PEERS` never authorizes egress. An
unset, empty, or non-empty value all route to the in-process adversarial
reviewer. Disclosure does not replace explicit reviewed authority.
...
The TraceWeaver-packaged `ce-code-review` invocation does not authorize an
external route. Do not announce or start a peer; report in Coverage that the
cross-model pass is held and the in-process adversarial reviewer was used.
