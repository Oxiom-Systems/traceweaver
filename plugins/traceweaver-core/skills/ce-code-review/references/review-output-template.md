<!-- TRACEWEAVER: file-role=packaged-skills-ce-code-review-references-review-output-template-md; req=REQ-TW-043; trace=TRACE-TW-009; ver=VER-TW-015 -->
...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `<temporary CE code-review artifact root>/<run-id>/`.
