...
candidate_amendment_status: authority_doc_review_passed_planning_input_behavior_held
owner: Oxiom Systems
---
...
| REQ-TW-053 | `tw-auto` must replace the alpha blanket no-publication ceiling with a controlled publication route. Requirements-quality review is required when requirements, authority, validation intent, release claims, or publication policy change; normal code publication does not require a new requirements gate when approved authority is unchanged. After implementation, verification, traceability update, and blocking-review closure pass, `tw-auto` may proceed to staging, commit, push, or PR creation only through TraceWeaver-packaged publication wrappers with a coherent staged tree, explicit publication target, credential/remote boundary checks, and either human confirmation for that target or a reviewed Intent Contract publication override. Any failed, missing, stale, untracked, unstaged, split authority/evidence state, unresolved requirement drift, or changed stakeholder intent must keep publication blocked and route back to requirements/authority review. | INTENT-TW-001, INTENT-TW-002, INTENT-TW-003, INTENT-TW-004, INTENT-TW-005, INTENT-TW-007, INTENT-TW-008 | Requirements review for changed authority, publication-route plan, `tw-auto`/publication-wrapper behavior fixture, negative publication smoke, active-host runtime proof, staged-tree coherence check, and authority doc review. | Can TraceWeaver automate normal verified code publication while still forcing requirements review when authority or intent changes? | User workflow feedback 2026-05-05; REQ-TW-039; REQ-TW-040; REQ-TW-041; REQ-TW-052 | candidate_reviewed_planning_input |
| REQ-TW-054 | TraceWeaver must maintain code/test trace anchors for behavior-bearing implementation and verification artifacts. Behavior-bearing source files must include a file-level trace anchor naming linked `REQ-TW-*` IDs, `TRACE-TW-*`/`VER-TW-*` IDs when known, and the file role. Public entrypoints, command handlers, orchestration functions, validators, runtime mutation helpers, publication helpers, and safety gates must include a nearby high-level trace anchor. Tests, fixtures, and smoke scripts must name the requirement and verification IDs they exercise. Line-by-line annotation is not required. Generated, vendored, purely cosmetic, mechanical, or externally owned files may omit anchors only when the matrix or an approved exception records why. TraceWeaver scanner/review behavior must flag missing anchors, stale requirement IDs, tests linked to superseded or missing requirements, tests without verification IDs, and implementation files without a matrix path as trace gaps or dead-TDD candidates before code review, engineering-complete, or release-ready claims pass. | INTENT-TW-001, INTENT-TW-003, INTENT-TW-008 | Code-trace anchor scanner, matrix code-anchor table, fixture cases, `tw-code-review` preflight, and doc/code review evidence. | Can contributors navigate from requirement to behavior-bearing code and TDD/verification evidence without noisy line-by-line annotations, and can TraceWeaver detect dead TDD code? | User workflow feedback 2026-05-05; dogfood audit gap for code-level annotations; REQ-TW-038; REQ-TW-046; REQ-TW-049 | candidate_reviewed_planning_input |
...
| REQ-TW-056 | `tw-auto` must accept a feature task, bug task, or plan path and run the full TraceWeaver closure loop internally so users do not need to manually alternate `/ce:work`, `/ce-code-review`, `/ce-doc-review`, and follow-up `/ce:work` commands. The loop must load or bootstrap authority, run `tw-authority-gate`, execute implementation through TraceWeaver-packaged `ce-work`, update trace/matrix/evidence records, run `tw-traceability-check`, run `tw-code-review` for behavior/code changes, run `tw-doc-review` for authority/document changes, classify findings, repair findings through bounded work cycles, refresh hashes and staged-tree identity when applicable, and stop only at a clean accepted state, an explicit held/blocked state with next action, or a human-decision pause. If requirements are unclear, contradictory, incomplete, missing, or need a material authority change, `tw-auto` must pause for user clarification or requirements review instead of changing requirements, inventing authority, or continuing implementation from an agent assumption. It must not hide review findings, bypass TraceWeaver wrappers, publish work, or claim clean replacement/runtime parity unless the separate controlled publication and runtime gates pass. | INTENT-TW-001, INTENT-TW-003, INTENT-TW-004, INTENT-TW-005, INTENT-TW-007, INTENT-TW-008 | `tw-auto` closure-loop plan, deterministic loop fixtures including human-decision pauses, review-wrapper behavior fixtures, trace/matrix write proof, authority doc review, active-host runtime transcript, and publication-route proof when publication is requested. | Can a user hand `tw-auto` a task or plan and get a completed or explicitly held TraceWeaver work package without manually driving the work/review/fix/review cycle, while unclear or contradictory requirements pause for user input rather than being silently changed? | User workflow feedback 2026-05-05; REQ-TW-033; REQ-TW-035; REQ-TW-037; REQ-TW-039; REQ-TW-049; REQ-TW-050; REQ-TW-053; REQ-TW-055 | candidate_reviewed_planning_input |
...
| REQ-TW-051 | CE 3.5.0 source worktree at upstream commit `1f3c6466e4eb4e1b584c658953dfb1ca98dd3335` compared against `plugins/traceweaver-core/skills/` | Candidate CE full-surface classification requirement records 20 unclassified/missing CE skills by name from a clean 3.5.0 source surface | Authority doc review passed / planning input only / behavior held | Per-surface classification/proof and wrapper/out-of-scope decisions remain required before full parity or clean replacement claims |
...
| REQ-TW-053 | User workflow feedback on 2026-05-05, `plugins/traceweaver-core/skills/tw-auto/SKILL.md`, packaged publication-wrapper candidates, and future publication-route proof | Candidate controlled publication-route requirement recorded so `tw-auto` can eventually automate staging, commit, push, and PR after normal code verification/review passes, while routing requirement/authority changes back through requirements review | Authority doc review passed / planning input only / behavior held | Publication-route plan, negative publication smoke, publication-wrapper behavior proof, credential/remote boundary proof, and human/Intent Contract override policy remain required |
| REQ-TW-054 | User workflow feedback on 2026-05-05 and dogfood audit gap for missing code-level anchors | Candidate code/test trace-anchor requirement recorded so behavior-bearing files, high-level functions, tests, fixtures, and smoke scripts can be linked to requirements and verification IDs without line-by-line annotation | Authority doc review passed / planning input only / behavior held | Code-anchor scanner plan, matrix code-anchor table, fixture proof, `tw-code-review` preflight update, and dead-TDD detection evidence remain required |
...
| REQ-TW-056 | User workflow feedback on 2026-05-05 that the manual work/code-review/doc-review/fix cycle is painful, plus follow-up clarification that `tw-auto` must pause for user input when requirements are unclear, contradictory, or lacking | Candidate `tw-auto` closure-loop requirement recorded so task/plan execution, TraceWeaver gates, CE work, trace updates, code review, doc review, finding repair, human-decision pauses, and clean/held stop classification can be orchestrated by `tw-auto` without letting the agent change requirements itself | Authority doc review passed / planning input only / behavior held | Closure-loop plan, deterministic fixtures for code/doc review finding repair, unclear/contradictory/missing requirement pauses, trace/matrix/evidence writes, runtime proof, and publication-route proof remain required |
...
1. Use REQ-TW-049 through REQ-TW-056 as reviewed planning input. The
   constrained active-host `tw-traceability-check` structured runtime recording
   for REQ-TW-055 is accepted only for the bounded fixture proof. Then implement
   and prove the review-wrapper, CE surface classification, wrapper
   backlog, controlled-publication, code-anchor, broader structured-finding,
   and `tw-auto` closure-loop behavior gates before claiming runtime/workflow
   authority.
