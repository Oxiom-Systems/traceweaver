...
| VER-TW-080 | REQ-TW-079 | Hook manifest/payload inspection | Static JSON schema validation against future per-host payload fixtures at `fixtures/hooks/session-start/claude/`, `fixtures/hooks/session-start/cursor/`, and `fixtures/hooks/session-start/codex/` (to be authored), each expressing the expected mode/Authority-Rule/held-claims JSON shape; a future smoke `scripts/traceweaver-smoke-hook-session-start` (to be authored) | SessionStart-equivalent hook manifests for Claude Code, Cursor, Codex | Not started | Hook manifest/payload files under a future `plugins/traceweaver-core/hooks/session-start/` path; `fixtures/hooks/session-start/{claude,cursor,codex}/`; `scripts/traceweaver-smoke-hook-session-start` (all to be authored) | 2026-07-22 | Placeholder allocation for Unit 1; no hook files exist yet. |
