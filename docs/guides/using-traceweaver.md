...
`traceweaver-core--v0.2.7` tag when you need a reproducible alpha snapshot.
...
```sh
git clone --branch traceweaver-core--v0.2.7 --depth 1 git@github.com:Oxiom-Systems/traceweaver.git
cd traceweaver
...
Antigravity support in `0.2.7` is local alpha install/discovery metadata only.
...
Cursor install/update is compatibility-preview only in `0.2.7`. The manifest is
